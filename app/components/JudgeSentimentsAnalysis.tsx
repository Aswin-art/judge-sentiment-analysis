...
/* ====================== Splash Screen Component (fixed hydration) ====================== */
const SplashScreen: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  const [progress, setProgress] = useState(0);
  const [loadingText, setLoadingText] = useState('Memuat sistem...');
  // Partikel disimpan di state dan dibuat hanya setelah mount
  const [particles, setParticles] = useState<
    { left: string; top: string; delay: string; duration: string }[]
  >([]);

  useEffect(() => {
    // generate partikel di client (bukan saat SSR)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900">
      {/* Animated background particles (hanya muncul setelah mount) */}
      ...
      <div className="text-center z-10">
        ...
        <div className="relative mb-8">
          {/* Outer ring */}
          <div
            className="absolute inset-0 w-32 h-32 border-4 border-white/20 rounded-full animate-spin mx-auto"
            style={{ animationDuration: '3s' }}
          />
          {/* Inner ring */}
          <div
            className="absolute inset-2 w-28 h-28 border-2 border-blue-300/40 rounded-full animate-spin mx-auto"
    ...
    >
      ...
      <main className="container mx-auto px-6 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            <div
              className={`rounded-2xl p-6 shadow-xl backdrop-blur-sm transition-colors ${
            ...
            >
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex items-center space-x-4">
                  <span
                    className={`text-sm ${
                      inputText.length > 5000 ? 'text-red-500' : darkMode ? 'text-slate-400' : 'text-slate-500'
                    }`}
                  >
          ...
          {result && (
            <div
              className={`rounded-2xl p-6 shadow-xl backdrop-blur-sm transition-all duration-500 ${
                darkMode ? 'bg-slate-800/50 border border-slate-700' : 'bg-white/70 border border-slate-200'
              }`}
            >
                    ...
                    <div>
                      <h4
                        className={`text-xl font-bold ${
                          result.isViolation ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'
                        }`}
                      >
                        {result.isViolation ? 'Terindikasi Pelanggaran' : 'Tidak Terindikasi Pelanggaran'}
                      </h4>
                      <p className={`${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                        Analisis berdasarkan KUHP dan UU ITE
                      </p>
                    </div>
                  </div>
                  ...
                  {result.isViolation && (
                    <div className={`mt-4 text-sm ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                      ...
                      <ul className="list-disc ml-5 mt-1 space-y-1">
                        {pickIndicativeTexts(result, 3).map((t, i) => (
                          <li key={i}>"{t}"</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="space-y-4">
                    {result.violationDetails.map((cat, i) =>
                      cat.laws.map((law, j) => (
                        <div
                          key={`${i}-${j}`}
                          className={`rounded-lg border-l-4 border-red-500 overflow-hidden ${
                        ...
                        >
                          ...
                          <div className="p-4 space-y-3">
                            {cat.texts.map((text, k) => (
                              <div
                                key={k}
                                className={`p-4 rounded-lg border-l-2 border-slate-300 ${
                              ...
                              >
                                ...
                                <blockquote
                                  className={`text-sm italic mb-3 pl-3 border-l-2 ${
                                    darkMode ? 'text-red-300 border-red-500' : 'text-red-800 border-red-300'
                                  }`}
                                >
                                ...
                                <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                  ...
                                  {text.includes(' (')
                                    ...
                                    : `Kalimat ini mengandung unsur ${cat.category.toLowerCase()} yang dapat melanggar ketentuan dalam ${
                                        law.split(' - ')[0]
                                      }`}
                                </p>
                              </div>
                ...
                <div className={`p-4 rounded-lg ${darkMode ? 'bg-slate-700/30' : 'bg-slate-50'}`}>
                  {result.isViolation ? (
                    <div className={darkMode ? 'text-slate-300' : 'text-slate-700'}>
                      <p className="mb-2">
                        Teks mengandung pelanggaran hukum dalam {result.violationDetails.length} pasal:
                      </p>
                      <ul className="list-disc ml-5 space-y-1">
                        {result.violationDetails.map((v, idx) => {
                          ...
                          return (
                            <li key={idx}>
                              <span className="font-medium">{pasal}</span>
                              {short ? `: ${short}` : null}
                            </li>
                          );
                        })}
